...
#Memory management simulation constants
MIN_MEMORY = 900
MAX_MEMORY = 1100
MAX_RETRIES = 5
...
used_memory = 0
memory_allocation_running = False
allocated_memory = []

def simulate_memory_usage(size):
    global used_memory
    used_memory += size
...
def allocate_memory(client_socket, username):
    """Request additional memory from the server."""
    global used_memory, memory_allocation_running, allocated_memory
    retries = 0
    start_time = time.time()

    while retries < MAX_RETRIES and (time.time() - start_time) < 60:
        memory_size = random.randint(MIN_MEMORY, MAX_MEMORY)  # Randomize memory size
        allocate_request = f"ALLOCATE:{username}:{memory_size}"
        client_socket.send(allocate_request.encode())
        response = client_socket.recv(1024).decode()
        
        if "Allocation successful" in response:
            used_memory += memory_size  # Update memory usage on success
            allocate_memory.append(memory_size)
            message_history.append(f"Server response: {response}")
            clear_prompt()
            break
        else:
            retries += 1
            message_history.append(f"Server response: {response} (Retry {retries})")
            clear_prompt()
        
        time.sleep(random.uniform(0.5, 2))  # Random delay between retries

    memory_allocation_running = False  # Stop allocation thread

def deallocate_memory(client_socket, username):
    """Deallocate all memory allocated for the user."""
    global allocated_memory
    while allocated_memory:
        memory_size = allocated_memory.pop(0)  # Remove from the tracking list
        deallocate_request = f"DEALLOCATE:{username}:{memory_size}"
        client_socket.send(deallocate_request.encode())
        response = client_socket.recv(1024).decode()
        message_history.append(f"Server response: {response}")
        clear_prompt()
...
def listen_for_messages(client_socket, stop_event):
    ...
    global memory_allocation_running
    ...
    while not stop_event.is_set():
        try:
            ...
            if message:
                ...
                if simulate_memory_usage(len(message)) and not memory_allocation_running:
                    memory_allocation_running = True
...
def start_client():
    ...
    while True:
        ...
        message_history.append(f"Connected to server on {host}:{port}")
        clear_prompt()
                ...
                if response == "Authentication successful!":
                    ...
                    while messageListener:
                        ...
                        if logged_in_action in {"logout", "l"}:
                            message_history.append("Deallocating memory...")
                            clear_prompt()
                            deallocate_memory(client_socket, username)
                            stop_event.set()
                            if stop_event.is_set:
                                messageListener = False
                                logout_message = f"LOGOUT:{username}"
                                client_socket.send(logout_message.encode())
                                message_history.append("Logged out successfully.")
                                clear_prompt()
                                break
