...
class Monitor:
    ...
    def allocate_memory(self, size):
        ...
        address = "none"
        response = {}
        for memory_location in self.memory:
            if self.memory[memory_location]["size"] >= size and not self.memory[memory_location]["used"]:
                address = memory_location
                self.memory[memory_location]["used"] = True
                break
        if address == "none":
            response = {
                "action": "allocate",
                "status": "failure",
                "message": "No available memory found for this size"
            }
        else:
            response = {
                "action": "allocate",
    ...
    def deallocate_memory(self, address):
        self.enter_monitor()
        response = {}
        if not self.memory[address]["used"]:
            response = {
                "action": "deallocate",
                "status": "failed",
                "message": "Memory at location not currently allocated"
            }
        else:
            self.memory[address]["used"] = False
            response = {
                "action": "deallocate",
                "status": "success"
            }
...
def handle_client(client_socket, client_address):
    ...
    while True:
        try:
            ...
            # Process ALLOCATE command
            elif data['action'] == "allocate":
                _size = data['size']
                response = MONITOR.allocate_memory(_size)
            
            # Process DEALLOCATE command
            elif data['action'] == "deallocate":
                _address = data['address']
                response = MONITOR.deallocate_memory(_address)
